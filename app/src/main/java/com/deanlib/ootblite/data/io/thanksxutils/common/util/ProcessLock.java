package com.deanlib.ootblite.data.io.thanksxutils.common.util;
...
public final class ProcessLock implements Closeable {
    ...
    static {
        File dir = OotbConfig.app().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE);
        ...
        synchronized (LOCK_MAP) {
            if (fileLock != null) {
                try {
                    ...
                    IOUtils.closeQuietly(fileLock.channel());
                }
            }
    ...
    private static ProcessLock tryLockInternal(final String lockName, final String hash, final boolean writeMode) {
        synchronized (LOCK_MAP) {
            ...
            try {
                File file = new File(
                        OotbConfig.app().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE),
                        hash);
                ...
                DLog.d("tryLock: " + lockName + ", " + ignored.getMessage());
                IOUtils.closeQuietly(stream);
