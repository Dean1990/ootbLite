...
public final class LruDiskCache {
    ...
    private LruDiskCache(String dirName) {
        this.cacheDb = OotbConfig.getDb(DbConfigs.HTTP.getConfig());
        this.cacheDir = OotbConfig.app().getCacheDir();
    ...
    public LruDiskCache setMaxSize(long maxSize) {
        if (maxSize > 0L) {
            long diskFreeSize = FileUtils.getDiskAvailableSize();
        ...
        if (cacheFile.getName().endsWith(TEMP_FILE_SUFFIX)) { // is temp file
            ...
            try {
                ...
                if (result == null) {
                    ...
                    IOUtils.deleteFileOrDir(destFile);
                } else {
                    IOUtils.closeQuietly(cacheFile);
                    IOUtils.deleteFileOrDir(cacheFile);
                }
            }
        } else {
            result = cacheFile;
        ...
        trimExecutor.execute(new Runnable() {
            ...
            public void run() {
                if (available) {
                    ...
                    // trim disk
                    try {
                        while (FileUtils.getFileOrDirSize(cacheDir) > diskCacheSize) {
                            List<DiskCacheEntity> rmList = cacheDb.selector(DiskCacheEntity.class)
    ...
    private boolean deleteFileWithLock(String path) {
        ...
        try {
            ...
            if (processLock != null && processLock.isValid()) { // lock
                ...
                return IOUtils.deleteFileOrDir(file);
            }
        } finally {
            IOUtils.closeQuietly(processLock);
        }
